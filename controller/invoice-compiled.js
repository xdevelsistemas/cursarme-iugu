...
module.exports = () => {
    ...
    controller.showInvoice = (req, res) => {
        if (parcelas.getUnidade(req.params.unidade)) {
            ...
            iuguInvoice.getInvoice(parcelas.getUnidade(codUnidade), invoiceId).then(invoice => {
    ...
    controller.invoiceDueDate = (req, res) => {
        ...
        if (parcelas.getUnidade(req.params.unidade)) {
            res.render('invoice', { unidade: req.params.unidade,
    ...
    controller.invoiceStatusChange = (req, res) => {
        ...
        if (req.body.data && req.body.data.id && req.body.event && req.body.data.status && req.body.event == 'invoice.status_changed' && req.body.data.status == 'paid' && parcelas.getUnidade(req.params.unidade)) {

            let invoiceid = req.body.data.id;
            let unidade = parcelas.getUnidade(req.params.unidade);

            iuguInvoice.getInvoice(unidade, invoiceid).then(response => {
    ...
    controller.processInvoice = (req, res) => {
        if (parcelas.getUnidade(req.params.unidade)) {
            ...
            parcelas.invoiceControllerCall(codUnidade, codMovimento, parcela).then(() => res.status(201).json("transacao criada")).catch(err => {
                if (err.message) {
    ...
    controller.processAllInvoices = (req, res, regenerate) => {
        if (parcelas.getUnidade(req.params.unidade)) {
            ...
            parcelas.getInvoices(month, year, parcelas.getUnidade(unidade).query, regenerate).then(recordset => {
                let totalItens = recordset.length;
                return Promise.reduce(recordset, (info, el) => {
                    return parcelas.invoiceControllerCall(el.CodUnidade, el.CodMovimento, el.Parcela).then(() => {
                        console.log('----------------------');
                        console.log('{} de {}'.format(info.total, recordset.length));
                        console.log('----------------------');
                        return { total: info.total + 1, invoiceWithErrors: info.invoiceWithErrors };
                    }).catch(err => {
                        console.log('problema com o boleto CodUnidade = {} /  CodMovimento = {} / Parcela = {} : {}'.format(el.CodUnidade, el.CodMovimento, el.Parcela, err.message));

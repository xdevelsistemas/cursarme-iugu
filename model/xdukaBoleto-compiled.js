...
let invoiceControllerCall = (codUnidade, codMovimento, parcela) => {
    ...
    if (getUnidade(codUnidade)) {
        ...
        return getInvoice(codUnidade, codMovimento, parcela).then(invoices => {
            return Promise.all([iuguInvoice.createInvoice(getUnidade(codUnidade), invoices[0]), iuguInvoice.cancelInvoice(getUnidade(codUnidade), invoices[0].xDevCobId)]);
        }).then(processed => {
...
let getInvoices = (month, year, unidadeQuery, reprocess) => connect(config).then(() => {
    let query = `select *
    ...
    return new mssql.Request().query(query.format(month, year, unidadeQuery));
}).then(result => {
    mssql.close();return Promise.resolve(result);
}).catch(err => {
    mssql.close();throw err;
});

let addTransaction = (transactId, transactUrl, CodUnidade, CodMovimento, Parcela) => connect(config).then(() => {
    return new mssql.Request().query(`INSERT into
                        cntMovimentoParcelaIugu
                        (CodUnidade, CodMovimento, Parcela, xDevCobId, xDevExternalUrl)
                        VALUES ({},{},{},'{}','{}')`.format(CodUnidade, CodMovimento, Parcela, transactId, transactUrl)).catch(err => {
        throw err;
    });
}).then(() => {
    return new mssql.Request().query(`UPDATE cntMovimentoParcela
                        set xDevCobId = '{}',
                        xDevExternalUrl = '{}'
                        where CodUnidade = {}
                        and CodMovimento = {}
                        and Parcela = {}`.format(transactId, transactUrl, CodUnidade, CodMovimento, Parcela)).catch(err => {
        throw err;
    });
}).then(result => {
    mssql.close();return Promise.resolve(result);
...
let getInvoice = (CodUnidade, CodMovimento, Parcela) => connect(config).then(() => new mssql.Request().query(`select *
                        from xdevbi_financeiro where
                        CodUnidade = {}
                        and CodMovimento = {}
                        and Parcela = {}
                        `.format(CodUnidade, CodMovimento, Parcela))).then(result => {
    mssql.close();return Promise.resolve(result);
}).catch(err => {
    mssql.close();
    throw err;
...
let payInvoice = (transactId, payValue, payDate) => connect(config).then(() => new mssql.Request().query(`select *
                        from xdevbi_financeiro where
                        xDevCobId = '{}'
                        `.format(transactId))).then(result => {
    if (result && result.length > 0) {
        return Promise.resolve(result);
    } else {
        return new mssql.Request().query(`select *
    ...
    if (invoiceData && invoiceData.length > 0) {
        ...
        return new mssql.Request().query(`insert into cntMovimentoLog
